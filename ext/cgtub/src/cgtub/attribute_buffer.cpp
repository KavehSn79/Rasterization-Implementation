...
template<typename T>
AttributeBuffer<T>::AttributeBuffer()
    : m_buffer(0u)
    , m_elements(0u)
{
...
template<typename T>
void AttributeBuffer<T>::upload(std::span<T const> values)
{
...
template<typename T>
size_t AttributeBuffer<T>::elements() const
{
    return m_elements;
}

template<typename T>
GLuint AttributeBuffer<T>::gl() const
{
    return m_buffer;
}

#define IMPLEMENT_ATTRIBUTE_BUFFER(type) \
    template class AttributeBuffer<type>;
